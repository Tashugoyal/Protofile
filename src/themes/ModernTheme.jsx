  ...
  return (
    <div className="blog-theme blog-theme--modern">
      ...
      <div className="modern-hero-panel">
        ...
        <p className="modern-headline-text">{headline || 'Headline / Expertise'}</p>
        ...
        <div className="modern-social-strip">
          ...
          {linkedinUrl && (
            <span className="modern-social-item">
              ...
              <a href={formatExternalUrl(linkedinUrl)} target="_blank" rel="noreferrer">LinkedIn</a>
            </span>
          )}
        </div>
      </div>
      ...
      <div className="modern-tab-content">
        <div className="modern-portfolio-grid">
          ...
          {projects && projects.length > 0 && (
            <section className="modern-glass-card projects-card" style={{ gridColumn: '1 / -1' }}>
              <h3 className="modern-card-title">Featured Projects</h3>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: '1.5rem' }}>
                {projects.map((proj, i) => (
                  <div key={i} style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid rgba(255,255,255,0.04)', borderRadius: '12px', padding: '1.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: '1rem', marginBottom: '0.5rem' }}>
                        <h4 style={{ color: '#ffffff', fontSize: '1.1rem', fontWeight: 700 }}>{proj.title}</h4>
                        {proj.link && (
                          <a href={formatExternalUrl(proj.link)} target="_blank" rel="noreferrer" style={{ fontSize: '0.8rem', color: '#818cf8', fontWeight: 600 }}>
                            GitHub 🔗
                          </a>
                        )}
                      </div>
                      <p style={{ color: '#94a3b8', fontSize: '0.85rem', lineHeight: '1.5', margin: '0.5rem 0 1rem' }}>{proj.description}</p>
                    </div>
                    {proj.techStack && (
                      <div>
                        <span style={{ fontSize: '0.75rem', background: 'rgba(99, 102, 241, 0.12)', color: '#c7d2fe', padding: '0.25rem 0.5rem', borderRadius: '6px', fontWeight: 700, display: 'inline-block' }}>
                          {proj.techStack}
                        </span>
                      </div>
                    )}
                ...
                {experience.map((exp, i) => (
                  <div key={i} className="modern-timeline-node">
                    ...
                    <div className="node-body">
                      <h4>{exp.title}</h4>
                      <h5>{exp.company} <span className="node-time">({exp.startDate} – {exp.endDate || 'Present'})</span></h5>
              ...
              <div className="modern-education-list">
                {education.map((edu, i) => (
                  <div key={i} className="modern-edu-item">
                    ...
                    <span className="node-time">{edu.startDate} – {edu.endDate}</span>
                  </div>
                ))}
              </div>
            </section>
